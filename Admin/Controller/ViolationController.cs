    ...
    class ViolationController
    {
        ...
        public List<Violation> GetViolations()
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                {
                    ViolationId = Convert.ToInt32(row["violation_id"]),
                    UserId = Convert.ToInt32(row["user_id"]),
                    DeviceId = Convert.ToInt32(row["device_id"]),
                    borrowId = Convert.ToInt32(row["borrow_id"]),
                    ReservationsId = Convert.ToInt32(row["reservations_id"]),
                    ViolationType = row["violation_type"].ToString(),
                    ViolationDescription = row["violation_description"].ToString(),
        ...
        {
            string query = "SELECT * FROM violations WHERE violation_id = @violationId";
        ...
        {
            string query = "INSERT INTO violations (user_id, device_id, borrow_id, reservations_id, violation_type, violation_description, date_reported, status, fine_amount) " +
        ...
        {
            string query = "UPDATE violations SET user_id = @userId, device_id = @deviceId, borrow_id = @borrowId, reservations_id = @reservationsId, violation_type = @violationType, violation_description = @violationDescription, date_reported = @dateReported, status = @status, fine_amount = @fineAmount WHERE violation_id = @violationId";
            MySqlParameter[] parameters = new MySqlParameter[]
            {
                new MySqlParameter("@userId", violation.UserId),
                new MySqlParameter("@deviceId", violation.DeviceId),
        ...
        {
            string query = "DELETE FROM violations WHERE violation_id = @violationId";
            MySqlParameter[] parameters = new MySqlParameter[]
            {
